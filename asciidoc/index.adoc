...
- pld.* - span between two point of time: when we submitted payload into reactor and when result is completed.
- exc.* - some time graph contains detached nodes (Detached Handler without Merger). In such cases graph execution is continues even after result completion. exc metric shows when whole graph execution is completes including such detached vertices.
- hndl.*
- mrg.*
...
ru.fix.completable.reactor.runtime.exc.AvailableSubscriptionsPayload.callsCountSum
ru.fix.completable.reactor.runtime.hndl.AvailableSubscriptionsPayload.fetchAbonentRestriction.activeCallsCountMax
ru.fix.completable.reactor.runtime.mrg.AvailableSubscriptionsPayload.fetchAbonentRestriction.activeCallsCountMax
ru.fix.completable.reactor.runtime.pld.AvailableSubscriptionsPayload.activeCallsCountMax
